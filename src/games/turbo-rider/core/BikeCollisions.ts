...
/**
 * Resolves overlap between human players' bikes. Never crashes, never touches lives/z —
 * only ever nudges lateral position and speed. See plan section E for the full rationale:
 * a single i<j pass (not TrafficManager's nested-forEach-over-two-arrays shape, which would
 * double-count every pair), minimum-translation-vector axis choice for side-vs-rear, and
 * z left untouched so a bump can never reorder race standings.
 */
