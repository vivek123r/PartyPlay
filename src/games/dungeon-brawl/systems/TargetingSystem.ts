...
export type Hostile = Enemy | DungeonBoss;

export interface TargetResult { id?: string; x: number; y: number; angle: number; distance: number; }

export class TargetingSystem {
  public findNearest(hero: Hero, enemies: Enemy[], boss: DungeonBoss | null, range: number): TargetResult {
  ...
  public findCluster(hero: Hero, enemies: Enemy[], boss: DungeonBoss | null, range: number): TargetResult {
    const candidates: Hostile[] = [...enemies.filter(enemy => enemy.hp > 0), ...(boss && boss.hp > 0 ? [boss] : [])];
