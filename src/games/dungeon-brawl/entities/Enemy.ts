...
const STATS: Record<Exclude<EnemyType, 'minotaur_boss'>, { hp: number; speed: number; damage: number; color: number; radius: number }> = {
...
export class Enemy implements EnemyState {
  ...
  constructor(id: string, type: Exclude<EnemyType, 'minotaur_boss'>, x: number, y: number, healthMultiplier = 1) {
  ...
  public render(g: Graphics, clock: number): void {
    ...
    if (this.type === 'skeleton') { g.rect(x - 6, y - 10, 12, 15).fill({ color: this.color }); g.rect(x - 5, y - 18, 10, 8).fill({ color: 0xf2f4eb }); g.rect(x - 3, y - 15, 2, 2).fill({ color: 0x24142f }); g.rect(x + 1, y - 15, 2, 2).fill({ color: 0x24142f }); g.rect(x + 7, y - 9, 2, 14).fill({ color: 0xb9a77e }); }
    else if (this.type === 'goblin') { g.rect(x - 5, y - 10, 10, 15).fill({ color: this.color }); g.rect(x - 7, y - 16, 14, 8).fill({ color: 0x5cae6a }); g.poly([x - 7, y - 15, x - 12, y - 20, x - 6, y - 19]).fill({ color: this.color }); g.poly([x + 7, y - 15, x + 12, y - 20, x + 6, y - 19]).fill({ color: this.color }); }
    else if (this.type === 'slime' || this.type === 'mini_slime') { g.ellipse(x, y - 4 - hop, this.radius, this.radius * 0.72).fill({ color: this.color }); g.rect(x - Math.max(3, this.radius - 5), y - 7 - hop, 2, 2).fill({ color: 0xffffff }); g.rect(x + Math.max(1, this.radius - 7), y - 7 - hop, 2, 2).fill({ color: 0xffffff }); }
    else if (this.type === 'imp') { g.rect(x - 5, y - 10, 10, 14).fill({ color: this.color }); g.poly([x - 7, y - 10, x - 13, y - 19, x - 4, y - 15]).fill({ color: 0x902d4b }); g.poly([x + 7, y - 10, x + 13, y - 19, x + 4, y - 15]).fill({ color: 0x902d4b }); g.circle(x, y - 14, 5).fill({ color: 0xffc077 }); }
    else if (this.type === 'wraith') { g.poly([x - 9, y + 4, x + 9, y + 4, x + 5, y - 16, x, y - 22, x - 5, y - 16]).fill({ color: this.color, alpha: 0.86 }); g.rect(x - 4, y - 14, 2, 2).fill({ color: 0x6ff7ff }); g.rect(x + 2, y - 14, 2, 2).fill({ color: 0x6ff7ff }); }
    else { g.rect(x - 10, y - 13, 20, 18).fill({ color: this.color }); g.rect(x - 8, y - 22, 16, 10).fill({ color: 0xd8b08b }); g.rect(x - 11, y - 17, 3, 22).fill({ color: 0x4f3240 }); g.rect(x + 10, y - 17, 3, 22).fill({ color: 0x4f3240 }); }
